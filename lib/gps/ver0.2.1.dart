...
class MyHomePageState extends State<MyHomePage> {
  ...
  Future<Timer?> _startGPS() async {
    ...
    if (!context.mounted) return timer;
    print('OK');
    TitleDialogState.inputDialog(context); //一番上でないとcontextが違うものになる。
...
class TitleDialogState extends State<TitleDialog> {
  ...
  static void _handleText() {
    MyHomePage.ofElementOfDialog().filenameInDialog = name;
    MyHomePage.ofElementOfDialog().dialogCompleted = true;
  }

  static Future<void> inputDialog(BuildContext context) async {
    //処理が重い(?)からか、非同期処理にする
